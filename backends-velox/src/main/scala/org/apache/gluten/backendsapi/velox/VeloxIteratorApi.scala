...
class VeloxIteratorApi extends IteratorApi with Logging {
  ...
  private def setFileSchemaForLocalFiles(
    ...
    // For ORC/DWRF, always attach the table schema so the native reader has a
    // target to remap file columns to. It is needed both when the whole scan is
    // mapped by position (orc.force.positional.evolution) and for the per-file
    // fallback where a file whose physical schema is all Hive placeholder names
    // (_col0, ...) is mapped by position even though ORC is read by name by
    // default, matching vanilla Spark's OrcUtils.requestedColumnIds.
