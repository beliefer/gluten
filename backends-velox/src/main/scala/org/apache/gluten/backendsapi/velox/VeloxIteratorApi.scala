...
class VeloxIteratorApi extends IteratorApi with Logging {
  ...
  private def setFileSchemaForLocalFiles(
    ...
    // all Hive placeholder names (_col0, ...) -- or when
    // orc.force.positional.evolution is set -- must be mapped by position even
    // though ORC is read by name by default, matching vanilla Spark's per-file
